...
url_izin="https://raw.githubusercontent.com/zhets/izinsc/main/ip"
username=$(curl $url_izin | grep $ipsaya | awk '{print $2}')
valid=$(curl $url_izin | grep $ipsaya | awk '{print $3}')
exp="${valid}"
clear
# CERTIFICATE STATUS
d1=$(date -d "$valid" +%s)
...
Exp1=$(curl $url_izin | grep $ipsaya | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
